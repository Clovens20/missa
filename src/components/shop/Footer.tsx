...
const PLATFORM_ICONS: Record<string, any> = {
  ...
  tiktok: (
    <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.9-.32-1.98-.23-2.81.33-.85.51-1.44 1.42-1.58 2.39-.16 1.17.45 2.39 1.44 3.03 1.11.72 2.59.75 3.75.11 1.12-.61 1.84-1.74 1.96-2.98.02-3.51.01-7.01.01-10.52.01-.13 0-.26 0-.39z"/>
    </svg>
  ),
  ...
  snapchat: (
    <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.001 0c-1.305 0-2.312.181-3.146.564-.812.373-1.421.921-1.89 1.636-.263.402-.454.858-.574 1.349-.12.492-.171 1.012-.171 1.576 0 .559.055 1.077.172 1.564.116.488.311.944.573 1.353.469.721 1.078 1.282 1.89 1.666.834.394 1.841.583 3.146.583 1.305 0 2.313-.189 3.146-.583.813-.384 1.421-.945 1.891-1.666.262-.409.457-.865.573-1.353.116-.487.172-1.005.172-1.564 0-.564-.052-1.084-.171-1.576-.119-.491-.311-.947-.574-1.349-.469-.715-1.078-1.263-1.89-1.636-.834-.383-1.841-.564-3.146-.564zM22.186 16.643c-.233-.243-.632-.501-1.215-.768-.584-.268-1.36-.554-2.34-.848-.287-.087-.535-.164-.75-.231-.214-.067-.384-.12-.511-.161-.127-.04-.213-.067-.257-.081-.044-.014-.047-.024-.009-.03 1.018-.152 1.761-.53 2.23-1.135.469-.605.703-1.4.703-2.385 0-.472-.055-.902-.165-1.291-.11-.389-.281-.724-.513-1.005-.233-.281-.536-.499-.91-.654-.373-.155-.827-.233-1.36-.233-.534 0-.987.078-1.361.233-.374.155-.677.373-.91.654s-.403.616-.513 1.005c-.11.389-.165.819-.165 1.291 0 .985.234 1.78.703 2.385.469.605 1.211.983 2.229 1.135.038.006.035.016-.009.03-.044.014-.13.041-.257.081s-.297.094-.511.161c-.215.067-.463.144-.75.231-.98.294-1.756.58-2.34.848s-.982.525-1.215.768c-.233.243-.35.539-.35.888 0 .151.025.292.075.422.05.131.127.247.231.35s.239.186.406.251c.167.065.367.098.599.098.232 0 .432-.033.599-.098s.302-.148.406-.251.181-.219.231-.35c.05-.131.075-.271.075-.422 0-.349-.117-.645-.35-.888z"/>
    </svg>
  ),
  linkedin: <Linkedin className="w-5 h-5" />,
  threads: (
    <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.001 0c-1.305 0-2.312.181-3.146.564-.812.373-1.421.921-1.89 1.636-.263.402-.454.858-.574 1.349-.12.492-.171 1.012-.171 1.576 0 .559.055 1.077.172 1.564.116.488.311.944.573 1.353.469.721 1.078 1.282 1.89 1.666.834.394 1.841.583 3.146.583 1.305 0 2.313-.189 3.146-.583.813-.384 1.421-.945 1.891-1.666.262-.409.457-.865.573-1.353.116-.487.172-1.005.172-1.564 0-.564-.052-1.084-.171-1.576-.119-.491-.311-.947-.574-1.349-.469-.715-1.078-1.263-1.89-1.636-.834-.383-1.841-.564-3.146-.564z"/>
    </svg>
  ),
}
...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/shop/footer').then(r => r.json()),
      fetch('/api/shop/legal-pages').then(r => r.json()),
      fetch('/api/shop/social-links').then(r => r.json()),
    ]).then(([footer, legal, social]) => {
      setFooterData(footer.data)
      setLegalPages(legal.data)
      setSocialLinks(social.data)
  ...
  const boutiqueLinks = footerData.boutique_links || []
  const aideLinks = footerData.aide_links || []
    ...
    <footer className="bg-gray-900 text-gray-300">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            <div className="relative w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center">
              <Image 
                src="/logo.png" 
                alt={brand.name || "Missa Shop Logo"} 
                fill 
            ...
            <div>
              <span className="font-black text-lg text-primary">
                {(getSetting('site_name') || brand.name || 'Missa').split(' ')[0]}
              </span>
              <span className="font-black text-lg text-secondary">
                {(getSetting('site_name') || brand.name || 'Shop').split(' ')[1] || ''}
              </span>
            </div>
          </div>
          <div 
            ...
            dangerouslySetInnerHTML={{ __html: getSetting('site_description') || brand.description || "Votre boutique en ligne premium." }}
          />
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {boutiqueLinks.map((l: any, i: number) => (
              <li key={i}>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            ...
            {/* Legal pages in help section - skip the first 2 which are in the bottom bar */}
            {legalPages.filter(p => p.show_in_footer).slice(2).map((page: any) => (
              <li key={page.slug}>
                <Link href={`/legal/${page.slug}`} className="hover:text-primary transition-colors">
                  {page.title}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-gray-800 py-6">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-gray-500">
          <p>{brand.copyright || `© ${new Date().getFullYear()} Missa Shop. Tous droits réservés.`}</p>
          <div className="flex gap-4">
            {legalPages.slice(0, 2).map((page: any) => (
              <Link 
                key={page.slug} 
