...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('orders_changes').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'guest_orders' }, (payload) => {
      ...
      if (Notification.permission === 'granted') { new Notification('🛒 Nouvelle commande!', { body: `${(payload.new as any).order_number} — ${formatAdminPrice((payload.new as any).total)}`, icon: '/logo.png' }) }
    }).subscribe()
