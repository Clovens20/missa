...
export default function OrderDetailPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden"><div className="px-6 py-4 border-b border-gray-800"><h2 className="font-black text-white">📦 Articles commandés</h2></div><div className="p-6 space-y-4">{order.items?.map((item: any, i: number) => (<div key={i} className="flex items-center gap-4 p-4 bg-gray-800 rounded-2xl"><div className="w-16 h-16 rounded-xl overflow-hidden bg-gray-700 flex-shrink-0">{item.image && <img src={item.image} alt={item.name} className="w-full h-full object-cover"/>}</div><div className="flex-1 min-w-0"><p className="font-bold text-white text-sm line-clamp-1">{item.name}</p>{item.variant && <p className="text-gray-500 text-xs mt-0.5">{item.variant.size && `Taille: ${item.variant.size}`}{item.variant.color && ` | Couleur: ${item.variant.color}`}</p>}<p className="text-gray-600 text-xs mt-0.5">SKU: {item.product_id?.substring(0, 8)}...</p></div><div className="text-right flex-shrink-0"><p className="text-white font-black">{formatPrice(item.price * item.qty)}</p><p className="text-gray-500 text-sm">{formatPrice(item.price)} × {item.qty}</p></div></div>))}</div><div className="px-6 pb-6"><div className="border-t border-gray-700 pt-4 space-y-2"><div className="flex justify-between text-sm text-gray-400"><span>Sous-total</span><span>{formatPrice(order.subtotal)}</span></div><div className="flex justify-between text-sm text-gray-400"><span>Livraison</span><span className={order.shipping === 0 ? 'text-secondary font-bold' : ''}>{order.shipping === 0 ? 'GRATUITE' : formatPrice(order.shipping)}</span></div><div className="flex justify-between text-sm text-gray-400"><span>Taxes</span><span>{formatPrice(order.tax)}</span></div><div className="flex justify-between font-black text-xl text-white pt-2 border-t border-gray-700 mt-2"><span>TOTAL</span><span className="text-primary">{formatPrice(order.total)}</span></div></div></div></div>
